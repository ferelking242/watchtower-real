use std::str::FromStr;
  ...
  /// A certificate verifier that accepts everything — used when verify_certificates = false.
  #[derive(Debug)]
  struct NoVerifier;
  ...
  /// Build a rustls ClientConfig that never calls rustls-platform-verifier.
  ///
  /// * When verify = true  → Mozilla CA bundle (webpki-roots), works on Android + iOS
  ///   without any JNI/Security-framework initialisation.
  /// * When verify = false → NoVerifier (SSL bypass for Cloudflare resolution etc.)
  ///
  ...
  fn build_tls_config(verify: bool, extra_certs: &[Vec<u8>]) -> rustls::ClientConfig {
      ...
      // Mozilla CA bundle — no platform-specific initialisation required.
      root_store.extend(webpki_roots::TLS_SERVER_ROOTS.iter().cloned());
      // Additional caller-supplied DER certificates.
      for cert_bytes in extra_certs {
          root_store
              .add(CertificateDer::from(cert_bytes.as_slice()))
              .ok();
      }
  ...
  fn create_client(settings: ClientSettings) -> Result<RequestClient, RhttpError> {
      let client: reqwest::Client = {
          // ── TLS ──────────────────────────────────────────────────────────────
          // Build a rustls config with Mozilla CA bundle so we never rely on
          // rustls-platform-verifier (panics on Android without JNI init, and
          // is not available on iOS when sideloaded via TrollStore).
          let (verify, extra_certs) = settings
              .tls_settings
              .as_ref()
              .map(|t| (t.verify_certificates, t.trusted_root_certificates.clone()))
              .unwrap_or((true, vec![]));
          let tls_cfg = build_tls_config(verify, &extra_certs);
          let mut client = reqwest::Client::builder().use_preconfigured_tls(tls_cfg);
          // ─────────────────────────────────────────────────────────────────────
          ...
          if let Some(tls_settings) = settings.tls_settings {
              // verify_certificates and trusted_root_certificates are handled above
              // via build_tls_config / use_preconfigured_tls.
