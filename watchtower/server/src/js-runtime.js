...
class WatchtowerRuntime {
  ...
  _ensureContext() {
    ...
    // CommonJS stubs so extensions that use module.exports don't crash
    const _moduleExports = {};
    const _module = { exports: _moduleExports };
    ...
    const sandbox = {
      ...
      require: (id) => { console.warn('[EXT] require() not supported:', id); return {}; },
      // Globals extensions expect
      global: null, // set after context creation via globalThis
      self: null,   // set after context creation via globalThis
    };

    this._context = vm.createContext(sandbox);

    // Fix self-references so extensions can use globalThis / global / self
    vm.runInContext('global = globalThis; self = globalThis;', this._context);
